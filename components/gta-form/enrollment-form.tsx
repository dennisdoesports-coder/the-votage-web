    ...
    >
      <div className="flex flex-col items-start justify-center pr-2 relative shrink-0">
        <div className="flex flex-col font-['Arial',sans-serif] justify-center leading-[0] not-italic relative shrink-0 text-[18px] md:text-[20px] text-black whitespace-nowrap">
          <p className="leading-[normal]">We would love to keep in touch with you through email</p>
        </div>
      </div>
    </motion.div>
  );
...
function EnrollmentFormModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const handleSubmit = () => {
    ...
    setTimeout(() => {
      alert(`Thank you, ${fullName}! Your enrollment has been submitted.`);
      setFullName('');
      setPhoneNumber('');
      setEmail('');
