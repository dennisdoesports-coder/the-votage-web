  ...
  const checkLocation = useCallback(
    (isRetryAction = false) => {
      ...
      const options: PositionOptions = {
        enableHighAccuracy: GEOFENCE_CONFIG.highAccuracy,
        timeout: GEOFENCE_CONFIG.timeoutMs,
        ...
        (error) => {
          ...
          switch (error.code) {
            ...
            case error.POSITION_UNAVAILABLE:
              setStatus('position_unavailable');
              setErrorMessage(
                'Location signal is unavailable. Please ensure GPS/Location is enabled on your device.'
              );
              break;
            case error.TIMEOUT:
              setStatus('timeout');
    ...
    [isBypassed, zones, processCoordinates]
  );
